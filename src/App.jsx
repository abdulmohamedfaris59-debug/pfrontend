...
function App() {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/products`, {
        ...
        image_url: productForm.image_url && productForm.image_url.trim()
  ...
  const getProductImage = (product) => {
    const imageType = product.image_url?.toLowerCase();

    if (imageType === "chicken") {
      return chickenImage;
    }
    if (imageType === "mutton") {
      return muttonImage;
  ...
  const placeOrder = async (event) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem("pickleCustomerPhone", customer.phone.trim());
  ...
  const loadMyOrders = async () => {
    ...
    try {
      const response = await axios.get(`${API_URL}/orders/customer/${orderPhone.trim()}`);
  ...
  const renderHomePage = () => (
    <>
      <section className="hero">
        <div>
          <span className="hero-tag">🌶️ 100% Homemade</span>
          <h2>
            Authentic Homemade
            <br />
            Non-Veg Pickles
          ...
          <p>
            Fresh ingredients. Traditional recipes. Delicious taste.
          </p>
    ...
    <section className="page-container">
      ...
      {cart.length === 0 ? (
        <div className="empty-cart">
          ...
          <p>Add your favorite pickles to start shopping.</p>
      ...
      <section className="page-container">
        ...
        <div className="checkout-layout">
          <form className="checkout-form" onSubmit={placeOrder}>
            ...
            <button type="submit" className="place-order-btn" disabled={placingOrder}>
  ...
  const renderSuccessPage = () => {
    ...
    const sendWhatsApp = () => {
      const whatsappNumber = "919789123392";

      let message = "🌶️ *NEW PICKLE ORDER* 🌶️\n\n";
      message += `*Order Number:* ${orderSuccess.order_number}\n\n`;
      message += "*CUSTOMER DETAILS*\n";
      message += "━━━━━━━━━━━━━━━━━━\n";
      message += `Name: ${orderSuccess.customer.name}\n`;
      message += `Phone: ${orderSuccess.customer.phone}\n`;
      message += `Address: ${orderSuccess.customer.address}\n\n`;
      message += "*ORDERED PRODUCTS*\n";
      message += "━━━━━━━━━━━━━━━━━━\n";

      orderSuccess.items.forEach((item, index) => {
        message += `\n${index + 1}. ${item.productName}\n`;
        message += `Quantity: ${item.quantity}\n`;
        message += `Price: ₹${Number(item.price).toFixed(2)}\n`;
        message += `Subtotal: ₹${Number(item.subtotal).toFixed(2)}\n`;
      });

      message += "\n━━━━━━━━━━━━━━━━━━\n";
      message += `Payment: Cash on Delivery\n`;
      message += `TOTAL: ₹${Number(orderSuccess.total_amount).toFixed(2)}\n`;
      message += "━━━━━━━━━━━━━━━━━━\n";
    ...
    return (
      <section className="success-page">
        ...
        <p>Thank you for ordering from Lyrah's SANUR.</p>

        <div className="success-order-card">
          <p>Order Number</p>
          <h3>{orderSuccess.order_number}</h3>
          <p>
            Status: <span className="status-pending">{orderSuccess.order_status}</span>
          </p>
        </div>
        ...
        <div className="success-order-card">
          ...
          <p>
            <strong>Name:</strong> {orderSuccess.customer.name}
          </p>
          <p>
            <strong>Phone:</strong> {orderSuccess.customer.phone}
          </p>
          <p>
            <strong>Address:</strong> {orderSuccess.customer.address}
          </p>
        </div>
        ...
        <div className="success-order-card">
          ...
          <div className="summary-total">
            ...
            <strong>₹{Number(orderSuccess.total_amount).toFixed(2)}</strong>
          </div>
        </div>
    ...
    return (
      <div className="modal-overlay" onClick={() => setShowAdminLogin(false)}>
        <div className="modal-content" onClick={(e) => e.stopPropagation()}>
          <button className="modal-close" onClick={() => setShowAdminLogin(false)}>
            ×
          </button>
          <h2>🔐 Admin Login</h2>
        ...
        <div className="modal-content product-modal" onClick={(e) => e.stopPropagation()}>
          ...
          <form onSubmit={isEditing ? handleUpdateProduct : handleAddProduct}>
            ...
            <div className="form-group">
              ...
              />
              <small>Use chicken, mutton or beef</small>
            </div>
            <button type="submit" className="admin-login-btn">
      ...
      return (
        <section className="page-container">
          <div className="admin-access-denied">
            <h2>⛔ Access Denied</h2>
            <button className="primary-btn" onClick={() => setShowAdminLogin(true)}>
              Admin Login
            </button>
          </div>
        </section>
    ...
    return (
      <section className="page-container">
        <div className="admin-header">
          ...
          <div className="admin-header-buttons">
            <button className="manage-products-btn" onClick={() => navigate("admin-products")}>
              📦 Manage Products
            </button>
            <button className="logout-btn" onClick={handleAdminLogout}>
              Logout
        ...
        <div className="admin-stats">
          ...
          <div className="stat-card">
            <h3>Pending</h3>
            <p>
              {allOrders.filter((o) => o.order_status === "Placed" || o.order_status === "Pending")
                .length}
            </p>
          </div>
          <div className="stat-card">
            <h3>Delivered</h3>
            <p>{allOrders.filter((o) => o.order_status === "Delivered").length}</p>
          </div>
          <div className="stat-card">
            <h3>Cancelled</h3>
            <p>{allOrders.filter((o) => o.order_status === "Cancelled").length}</p>
          </div>
        </div>
        ...
        {allOrders.length > 0 ? (
          <div className="orders-list admin-orders">
            {allOrders.map((order) => (
              <div className="my-order-card admin-order-card" key={order.id}>
                <div>
                  <p>Order Number</p>
    ...
    <section className="page-container">
      ...
      <div className="find-orders">
        <input
          ...
          placeholder="Enter your phone number"
        />
        <button onClick={loadMyOrders}>Find Orders</button>
      </div>
        ...
        <div className="orders-list">
          {myOrders.map((order) => (
            <div className="my-order-card" key={order.id}>
              ...
              <div>
                <p>Order Status</p>
                <span>{order.order_status || "Placed"}</span>
              </div>
              <div>
                <p>Payment</p>
                <span>💵 Cash on Delivery</span>
  ...
  const renderAboutPage = () => (
    <section className="page-container">
      <div className="about-card">
        <h2>About Lyrah's SANUR</h2>
        <p>
          We prepare delicious homemade non-vegetarian pickles using traditional recipes and quality
          ingredients.
        </p>
  ...
  return (
    <div className="app">
      <header className="header">
        <div className="header-inner">
          <button className="brand" onClick={() => navigate("home")}>
            <img src={logoImage} alt="Lyrah's SANUR" />
            <div>
              <h1>Lyrah's SANUR</h1>
              <p>Homemade Non-Veg Pickles</p>
            </div>
          </button>
          ...
          <nav className="nav">
            ...
            <button className={activePage === "orders" ? "active" : ""} onClick={() => navigate("orders")}>
              My Orders
            </button>
            <button
              className={activePage === "admin" || activePage === "admin-products" ? "active" : ""}
              onClick={() => navigate("admin")}
            >
              {isAdmin ? "👑 Admin" : "🔐 Admin"}
            </button>
            <button className={activePage === "contact" ? "active" : ""} onClick={() => navigate("contact")}>
              Contact
            </button>
            <button className={activePage === "about" ? "active" : ""} onClick={() => navigate("about")}>
              About
            </button>
      ...
      <main>
        ...
        {activePage === "checkout" && renderCheckoutPage()}
        {activePage === "success" && renderSuccessPage()}
        {activePage === "orders" && renderOrdersPage()}
        {activePage === "admin" && renderAdminOrdersPage()}
        {activePage === "admin-products" && renderAdminProductsPage()}
